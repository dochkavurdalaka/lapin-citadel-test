...
void PrintResult(const std::unordered_map<std::string, Sensor>& sensors_map,
...
int main(int argc, char* argv[]) {
    ...
    std::unordered_map<std::string, Sensor> sensors_map;
    std::unordered_map<std::string, Rule> rules_map;

    std::string prs_config = ReadConfig(&sensors_map, &rules_map);
    if (not prs_config.empty()) {
        std::cout << prs_config << std::endl;
        return 0;
    }

    LogProcessor log_processor(&rules_map, &sensors_map);
