...
class LogProcessorMultiThread {
    const std::unordered_map<std::string, Rule>& rules_map_;
    std::unordered_map<std::string, MultiThreadSensor>& sensors_map_;
    std::unordered_map<std::string, std::string> desc_to_name_map_;
    int count_threads_;

public:
    LogProcessorMultiThread(const std::unordered_map<std::string, Rule>* rules,
                            std::unordered_map<std::string, MultiThreadSensor>* sensors,
                            int count_threads)
        : rules_map_(*rules), sensors_map_(*sensors), count_threads_(count_threads) {
        for (const auto& [key, value] : sensors_map_) {
            desc_to_name_map_.emplace(value.GetDescription(), value.GetName());
        }
    }

    std::string ProcessFiles(const std::vector<fs::path>& file_pathes) {
        if (static_cast<size_t>(count_threads_) >= (file_pathes.size())) {
            std::string log_prs;
            ProcessFilesInThread(file_pathes.begin(), file_pathes.end(), &log_prs, 0);
            return log_prs;
        }
        std::vector<std::string> logs(count_threads_);

        size_t size = file_pathes.size() / count_threads_;

        std::vector<std::thread> threads;

        std::vector<fs::path>::const_iterator first = file_pathes.begin();
        std::vector<fs::path>::const_iterator last = file_pathes.end();
        for (int i = 0; i + 1 < count_threads_; ++i) {
            threads.emplace_back(&LogProcessorMultiThread::ProcessFilesInThread, this,
                                 first + i * size, first + (i + 1) * size, &logs[i], i);
        }
        ProcessFilesInThread(first + (count_threads_ - 1) * size, last, &logs.back(),
                             count_threads_ - 1);
    ...
    void ProcessFilesInThread(std::vector<fs::path>::const_iterator first,
                              std::vector<fs::path>::const_iterator last, std::string* log,
                              int thread_ind) {
        for (auto it = first; it != last; ++it) {
            ProcessFile(*it, log, thread_ind);
        }
    }

    void ProcessFile(const fs::path& file_path, std::string* log_parsing, int thread_ind) {
            ...
            if (trimmed_line.back() == ':') {
                ...
                for (auto it = rules_to_find.begin(); it != rules_to_find.end();) {
                    ...
                    if (std::regex_match(trimmed_line, matches, rule.pattern)) {
                        ...
                        auto& measure = sensor.GetMeasure(rule.name, thread_ind);
